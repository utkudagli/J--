...
enum TokenKind {
    EOF("<EOF>"), ABSTRACT("abstract"), BOOLEAN("boolean"), CHAR("char"), CLASS(
            ...
            "+"), ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), GE(">="), SRS(">>"), SRSE(">>="), ZFRS(">>>"), 
    		ZFRSE(">>>="), INC("++"), LAND("&&"), LE("<="), LNOT("!"), MINUS("-"), PLUS_ASSIGN("+="), STAR("*"),
    		DIV("/"), MOD("%"), LPAREN("("), RPAREN(")"), LCURLY("{"), RCURLY("}"), LBRACK("["), RBRACK(
            "]"), SEMI(";"), COMMA(","), DOT("."), IDENTIFIER("<IDENTIFIER>"), INT_LITERAL("<INT_LITERAL>"),
    		CHAR_LITERAL("<CHAR_LITERAL>"), STRING_LITERAL("<STRING_LITERAL>"), DOUBLE_LITERAL("<DOUBLE_LITERAL>"), 
    		LONG_LITERAL("<LONG_LITERAL>"), FLOAT_LITERAL("<FLOAT_LITERAL>"), BOOLEAN_LITERAL("<BOOLEAN_LITERAL>"), MINUS_ASSIGN("-="), ARROW("->"), LT("<"), 
    		SLS("<<"), SLSE("<<="), STAR_ASSIGN("*="), LNOTA("!="), DIVA("/="), BITWISECOMP("~"), BAND("&"), BANDA("&="),
    		COND("?"), BOR("|"), LOR("||"), BIOR("|="), COL(":"), BXOR("^"), BXORA("^="), MODA("%="), CONTINUE("continue"), 
    		FOR("for"), SWITCH("switch"), ASSERT("assert"), DEFAULT("default"), SYNCHRONIZED("synchronized"), 
    		DO("do"), GOTO("goto"), BREAK("break"), DOUBLE("double"), IMPLEMENTS("implements"), THROW("throw"), 
    		BYTE("byte"), THROWS("throws"), CASE("case"), ENUM("enum"), TRANSIENT("transient"), CATCH("catch"), 
    		SHORT("short"), TRY("try"), FINAL("final"), INTERFACE("interface"), VOLATILE("volatile"), STRICTFP("strictfp"),
    		LONG("long"), FINALLY("finally"), CONST("const"), FLOAT("float"), NATIVE("native"); 
