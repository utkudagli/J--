...
public class Parser {
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        return new JClassDeclaration(line, mods, name, superClass, classBody(), list);
    }
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods){
        ...
        if (have(EXTENDS)){
            if (have(FINAL)){
                reportParserError("Interface can't have final");
            }
            superClass = qualifiedIdentifier();
                while (have (COMMA)){
                superClass = qualifiedIdentifier();
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            JExpression test = parExpression();
            JStatement consequent = statement();
            return new JForStatement(line, forInit(), test, consequent, update());
        }else if (have(WHILE)) {
            JExpression test = parExpression();
            ...
            if (have(WHILE)){
                mustBe(SEMI);
                return new JDoWhileStatement(line, test, body);           
            }else {
                JStatement statement = statement();
                return new JDoWhileStatement(line, statement, body);
            }
        }else if(have (BREAK)){
            mustBe(IDENTIFIER);
            mustBe(SEMI);
    ...
    private JVariableDeclaration forInit(){
        Type type = type();
        ArrayList<JVariableDeclaration> statementExpression = new ArrayList<JVariableDeclaration>();
        while(have(COMMA)){
            statementExpression.add(forInit());
        }
        if (have(FINAL)){
            return localVariableDeclarationStatement();
        }
    ...
    private JVariableDeclaration update(){
        ...
        ArrayList<JVariableDeclaration> statementExpression = new ArrayList<JVariableDeclaration>();
        while(have (COMMA)){
            statementExpression.add(update());
        }
        return localVariableDeclarationStatement();
    }
    /**
    ...
    private JExpression conditionalOrExpression(){
        ...
        if (have(LOR)){
            return new JLorOp(line, lhs, conditionalOrExpression());
        }
        else {
            return lhs;
        }
